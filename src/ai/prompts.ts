...
3. meaning 是脱离语境的基础释义，简短（不超过 25 个字），可以给 1-2 个常见义项。
4. examples 里的每一句都必须是你新写的，用${reading === '用户正在阅读的外语' ? '原文所用的语言' : reading}书写，不能照抄原文，尽量贴合用户正在阅读的领域，每句 8-20 个词；多句之间要体现不同用法而不是同义重复。
5. phonetic 给 lemma（词典原形）的 IPA，用斜线包裹，如 /maɪˈɡreɪʃn/。不确定就返回空字符串，绝不编造。
6. cefr 给这个词本身的 CEFR 难度等级（A1/A2/B1/B2/C1/C2），判断依据是该词在通用语料中的
   常见程度，而不是它在本句中的用法。**拿不准就返回空字符串**，宁可不给也不要猜。
7. sentenceTranslation 是【所在句子】的完整翻译，通顺自然，不是逐词直译；只翻译那一句，不要翻译整段。
8. synonyms 给 2-4 个近义词，每个都要带 meaning：用一句话说明它的意思**以及它和该词的区别**。只给词不给释义等于没给。
9. 如果划选的是短语、习语或专有名词，kind 返回 "phrase"，phonetic 可以留空，重点解释整体含义而不是逐词直译。
10. meaning、contextMeaning、sentenceTranslation、exampleTranslation、synonyms[].meaning 一律用${target.name}书写，语气平实，不要客套、不要 markdown、不要 emoji。
11. 只输出 JSON，不要任何解释性前后缀。`
  }
...
3. meaning is the context-free dictionary sense, short, at most one or two senses.
4. Every entry in examples must be a NEW sentence you write in ${source.code === 'auto' ? 'the language of the source text' : source.name}, never copied from the page, ideally in the same domain the user is reading, 8-20 words each; when there is more than one they must show different uses, not paraphrase each other.
5. phonetic is the IPA of the lemma, wrapped in slashes. Return an empty string if unsure — never invent one.
6. cefr is the CEFR band of the word itself (A1-C2), judged by how common it is in general use, not by its use in this sentence. Return an empty string if genuinely unsure — never guess.
7. sentenceTranslation is a full, idiomatic translation of THE SENTENCE the word appeared in — that sentence only, not the paragraph.
8. synonyms: 2-4 near-synonyms, each with a meaning that says what it means AND how it differs from the headword. A bare word list teaches nothing.
9. For a phrase, idiom or proper noun, set kind to "phrase", leave phonetic empty if appropriate, and explain the whole rather than word by word.
10. Write meaning, contextMeaning, sentenceTranslation, exampleTranslation and every synonyms[].meaning in ${target.name}. Plain tone, no pleasantries, no markdown, no emoji.
11. Output JSON only, with no prose before or after it.`
}

const DETAIL_INSTRUCTION: Record<string, string> = {
  core: '【本次只要】word, lemma, kind, phonetic, partOfSpeech, cefr, meaning, contextMeaning, englishDefinition。不要输出例句、整句翻译或近义词——它们由另一次请求负责。',
